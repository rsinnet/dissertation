...
\begin{proof} [Proof of \thmref{theorem:main-theorem}]
  ...
  \begin{align}
    \Le =
    \left(\begin{array}{c c}
      \beta_{2} - r_{3} + \sigma (\beps - c_{1}) & \beta_{2} - r_{3} + \sigma \beps\\
      \beta_{2} - r_{3} + \sigma \beps & \beta_{2} - r_{3} + \sigma \beps
    \end{array}\right)
  \end{align}
  The matrix is positive definite if
  \begin{align}
    \nonumber
    \lefteqn{0 < \det \Le = - [\beta_{2} - r_{3} + \sigma \beps]^2}\\
    && \mbox{} + [\beta_{2} - r_{3} + \sigma \beps] [\beta_{2} - r_{3} + \sigma (\beps - c_{1})].
  \end{align}
  {\bf This matrix is NEVER positive definite.}
\end{proof}
