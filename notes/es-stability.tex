...
\begin{lemma}
  ...
  \begin{align}
    ...
    \nonumber
    \| \Pe\argsxz - \P\argsxz \| &\leq A \nzdxz,
  \end{align}
  where $\LTI$ and $A$ (not Lipschitz; see Thm 3.4, Khalil).
\end{lemma}
\begin{proof}
...
\begin{proof} [Proof of \thmref{theorem:main-theorem}]
  ...
  \end{align}
  %
  Using \lemref{lemma:TIe-bounds} and defining $\beps= \frac{c_{2}}{\resclfparam^{2}} \LDelta^{2} e^{-\frac{c_{3}}{\resclfparam} .9 \Tst}$ results in
  \begin{align}
    \label{eq:DVeX-bounds}
    \Vex(\Pex\argsxz) - \Vex(\zdx) \leq \beps \nzdxz^{2} - c_{1} \nzdx^{2}.
  \end{align}
  ...
  \begin{align}
    ...
    \nonumber
    &&r_{4} \| \Pe\argsxz - \P\argsxz \| \left( \|\P\argsxz\| + \|\Pe\argsxz\| \right).
  \end{align}
  Applying \eqref{eq:Pe-P-bounds} to the above results in
  \begin{align}
    \label{eq:Pe-P-beta}
    | \Vn(\Pe\argsxz) - \Vn(\P\argsxz) | \leq \beta_{2} \nzdxz^{2}
  \end{align}
  where, for simplicity, $\beta_{2} := r_{4} A (N \alpha + A  + \LP)$.
  %
  \begin{align}
  ...
  \begin{align}
    \Le =
    \left(\begin{array}{c c}
      \beta_{2} - r_{3} + \sigma (\beps - c_{1}) & 0\\
      0 & \beta_{2} - r_{3} + \sigma \beps
    \end{array}\right)
  \end{align}
  The matrix is positive definite if
  \begin{align}
    \nonumber
    \det \Le &= [\beta_{2} - r_{3} + \sigma \beps] [\beta_{2} - r_{3} + \sigma (\beps - c_{1})]\\
    &> 0
  \end{align}
  and, therefore, it is required that
  \begin{align}
    \beta_{2} - r_{3} + \sigma \beps > \sigma c_{1}
  \end{align}
  For small enough $\resclfparam$, $\beps > c_{1}$, and then for large enough $\sigma$, $\sigma(\beps - c_{1}) > \beta_{2} - r_{3}$.\qedhere
