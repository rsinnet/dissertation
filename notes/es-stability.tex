...
\begin{proof} [Proof of \thmref{theorem:main-theorem}]
  ...
  \end{align}
  %
  Using \lemref{lemma:TIe-bounds} and defining $\bepsone= \frac{c_{2}}{\resclfparam^{2}} \LDelta^{2} e^{-\frac{c_{3}}{\resclfparam} .9 \Tst}$ results in
  \begin{align}
  ...
  \begin{align}
    \Le =
    \left(\begin{array}{c c}
      \bepstwo - r_{3} + \sigma (\bepsone - c_{1}) & \bepstwo - r_{3} + \sigma \bepsone\\
      \bepstwo - r_{3} + \sigma \bepsone & \bepstwo - r_{3} + \sigma \bepsone
    \end{array}\right)
  \end{align}
  The matrix is positive definite if
  \begin{align}
    \lefteqn{0 < \det \Le = - [\bepstwo - r_{3} + \sigma \bepsone]^2}\\
    \nonumber
    && \mbox{} + [\bepstwo - r_{3} + \sigma \bepsone] [\bepstwo - r_{3} + \sigma (\bepsone - c_{1})].
  \end{align}
  Since, through simplification,
  \begin{align}
    \det \Le = -c_{1} (\bepstwo - r_{3} + \sigma \bepsone),
  \end{align}
  stability is only achieved if
  \begin{align*}
    &r_{3} > \bepstwo + \sigma \bepsone. \qedhere
  \end{align*}
\end{proof}
