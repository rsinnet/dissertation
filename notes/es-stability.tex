...
%
In other words, the zero dynamics manifold $\Zeta$ is the restricted subset of $X$ such that $x = 0$.
%
...
Exponential stability is guaranteed if a {\em control Lyapunv function} exists satisfying
\begin{align}
  c_{1} \| \eta \|^{2} \leq \V(\eta) \leq c_{2} \| \eta \|^2,\\
  L_{f} \V(\eta, \zeta) + L_{g} \V(\eta, \zeta) = 0.
