...
\begin{proof}
  ...
  %
  As a result, there exists a solution to the equality; that is,
  \begin{align}
    \lefteqn{\u(\q, \dq) = \B^{-1}(\q) \M(\q) \bigg[ \frac{c_{3}}{\resclfparam} \frac{1}{2} \E^{2}(\q, \dq)}\\
      \nonumber
      && \mbox{} + \pd{\E(\q, \dq)}{\q} \dq - \pd{\E(\q, \dq)}{\dq} \M^{-1}(\q) \CG(\q, \dq) \bigg]
  \end{align}
  %
  Now, for the CLF \eqref{eq:lyap-cond-nrg-es} with any locally Lipschitz continuous feedback control law $u : \zdX \times \zdZ \to \K$, it follows that solutions satisfy
  \begin{align*}
