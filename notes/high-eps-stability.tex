...
\begin{proof}
  ...
  \begin{align}
    \TB(0, 0, \zdx, \zdz) = \TI(x, z).
  \end{align}
  Since $\|\argsmuxz\| = \|\mu_{1}\| + \|\mu_{2}\| + \|\zdx\| + \|\zdz\|$, it follows that
  \begin{align}
    | \TB\argsmuxz - \TI\argsxz| \leq \LB (\| \mu_{1} \| + \| \mu_{2} \|),
  \end{align}
  where $\LB$ is the local Lipschitz constant. Defining
  \begin{align}
    \mu_{1} &= x_{1}(t)\big|_{t = \TIe\argsxz} - x_{2}(t)\big|_{t = \TIe\argsxz}\\
    \nonumber
    \mu_{2} &= z_{1}(t)\big|_{t = \TIe\argsxz} - z_{2}(t)\big|_{t = \TIe\argsxz}
  \end{align}
  results in
  \begin{align}
