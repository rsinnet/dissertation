...
%
Under the action of control effort $\uu$, the corresponding hybrid control system has the form
%
\begin{align}
  \HCS = \left\{
  \begin{array}{l l}
    \dx = \xf\arx + \xg\arx \, \uu, & \x \in \D \setminus \Guard,\\
    \xp = \Delta(\xm), & \x \in \Guard,
  \end{array}\right.
  \label{eq:hcsys}
\end{align}
...
\begin{definition} A {\em hybrid control system in a cycle} is a tuple,
  ...
  \item $FG = \{(f_{v}, g_{v})\}_{v \in E}$ with $(f_v,g_v)$ a {\em control system} on $D_{v}$, i.e., $\dot{x} = f_{v}(\x) + g_{v}(\x) u$ $\forall$ $(\x^{T}, \uu^{T})^{T} \in \D_{v}$.
  \end{itemize}
\end{definition}
...
The control system $\dx = f_v(\x) + g_v(\x) \, \uu$ can be explicity constructed through the constraints imposed on each domain by the domain breakdown.
%
...
\begin{align}
  \label{eq:domain}
  \D_{v} \! = \! \left\{ \! (q^{T}, \! {\dot q}^{T}, \! \uu^{T})^{T} \! \in \! T\Q \times \R^{m_{v}} : A_{v}\argsqdqu \! \geq \! 0 \! \right\}.
\end{align}
%
