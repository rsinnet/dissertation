...
\begin{definition} A {\em hybrid control system in a cycle} is a tuple,
  ...
  \item $\D = \{\Dv\}_{v \in V}$ is a set of {\em domains} with $\Dv
  ...
  \item $\Guard = \{\Guard_{e}\}_{e \in E}$ is a set of {\em guards} or {\em
      switching surfaces}, with $\Guard_{e} \subseteq \D_{\sor(e)}$,
  \item $\Delta = \{\Delta_{e}\}_{e \in E}$ is a set of {\em reset maps}, with
    $\Delta_{e} : S_{e} \to \mathcal{X}_{\tar(e)}$ a smooth map,
  \item $FG = \{(\xfv, \xgv)\}_{v \in E}$ with $(\xfv, \xgv)$ a {\em control
  ...
  for $v \in V$.
  % 
  The set of all domains can be written $\HCS = \{\HCSv\}_{v \in V}$.
\end{definition}

\begin{definition}
  A {\em hybrid system} is a hybrid control system with $U_v = \emptyset$
  $\forall$ $v \in V$, e.g., any relevant feedback controllers have been
  ...
  for $v \in V$.
  %
  The set of all domains can be written $\HS = \{\HSv\}_{v \in V}$.
\end{definition}
...
will be clarified further in \secref{sec:discrete-dynamics}.
...
%  \caption{A three-dimensional bipedal robot, together with world ($R_0$), body
%  ($R_b$) and reference ($R_c$) coordinate frames} \label{biped}
%\end{figure}
...
\begin{exmp} \label{ex:domainbreakdown}
  In the case of the graph $\Gamma_{u}$ given in \exmpref{universalgraph} and the set of contact points $\C =  \{\csh, \cst, \cnsh, \cnst\}$, for the graph shown in \figref{fig:domaingraph}, the domain breakdown is formally given by $\B_{u} : V_{u} \to \Z_{2}^{4}$ where $\Br_{u}(ts)$, $\Br_{u}(tl)$, $\Br_{u}(hl)$ and $\Br_{u}(hs)$ are given by
  \begin{align}
...
Given a vertex $v \in V$, the domain $\Dv$ is the set of admissible
