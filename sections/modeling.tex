...
Let $R_{0}$ be a fixed inertial or world frame and let $R_{b}$ be a reference
...
%
Let $\q_{s} \in \Q_{s}$ represent a choice of {\em body} or {\em shape
  coordinates} for the robot with $\Q_{s}$ the configuration space.
%
Herein, $\q_{s}$ represents the relative angles between adjacent links of the
robot as shown later in \figref{fig:shapecoords}.
%
The {\em generalized coordinates} are then found by combining the shape
coordinates with the position and orientation of the body-fixed frame, $R_{b}$,
viz.
%
\begin{align*}
  \q = (p_{b}^{T}, \phi_{b}^{T}, \q_{s}^{T})^{T} \in \Q = \R^{3} \times \SO(3)
  \times \Q_{s},
\end{align*}
%
...
%
The Euler-Lagrange equations can be used to find the equations of motion, which,
...
As mentioned, the dynamics of a rigid-body robot is governed by \eqref{eq:eom}
and this model takes accounts reaction forces but they do not appear in the
equations as they are included in the form of kinematic constraints which alter
the form of the dynamics.
%
In some cases, one may wish, for a variety of reasons, to compute the reaction
forces themselves.
%
For those of study materials and mechanism design of bipedal robots, it may be
...
has full row rank.
%
Intuitively, the rows of $J\argsq$ represent velocities of the systems which
are to be restricted to a constant; generally and all throughout this thesis,
...
%
The idea is to solve for $\lambda$ in terms of $\q$, $\dq$, and $u$ such that
\eqref{eq:holocon} is satisfied.
%
...
\end{align*}
Using this expression, the constraining forces acting on the system are given by
\begin{align*}
...
%
This thesis treats impacts using the model of Hurmuzlu and Marghitu
\cite{Hurmuzlu1994}.
%
An impact occurs when a rigid body (the foot) comes into contact with the
ground.
%
For simplicity, impacts are assumed to be rigid and plastic.\todo{More
  citations, look at Master's thesis.}
%
...
\eqref{eq:eom-lambda} results in\todo{Extended model.}
\begin{align}
  \label{eq:impulse}
  \De\argsqe(\dqep - \dqem) = J^{T}\argsqe \lambda\argsqedqem
\end{align}
where
\begin{align*}
  \int_{t^{-}}^{t^{+}} \! \lambda(\qe(\tau), \dqe(\tau)) \, d\tau.
\end{align*}
and $\dqem = \lim_{\tau \nearrow t} \dqe\argtau$ and $\dqep = \lim_{\tau
  \searrow t} \dqe\argtau$ are the left and right limits to the solution
$\dqe\argt$.
%
There may be a discrete jump and these limits may not be different due to
impulsive forces applied at impact.
%
Based on the desired constraints, one can construct a Jacobian matrix,
$J\argsqe$, whose rows contain individual constraints.
%
Combining \eqref{eq:eom-lambda} with \eqref{eq:impulse} results in the impact model:
\begin{align*}
  \left(\begin{array}{c c}
      \De\argsqe & -J^{T}\argsqe\\
      J\argsqe & \boldzero
    \end{array}\right)
  \left(\begin{array}{c}
      \dqep\\
...
of multidomain hybrid systems:\todo{notation of \cite{Guckenheimer1995}}
...
Contact points introduce {\it holonomic constraints}, $\eta_{c}$ for $c \in \C$,
